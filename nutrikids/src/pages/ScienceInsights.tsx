...
import {
  AGE_GROUPS, DEV_GOALS, DRI_CATEGORIES, DRI_NUTRIENTS, TIER_CONFIG,
  stageIdxForChild, type Tier,
} from '../data/growth';

/* 按 V4 原型的科学解读页：各年龄段发育目标 + 每日营养指南（联动年龄段选择） */
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} className="w-full h-auto select-none">
      ...
      {DRI_CATEGORIES.map((cat, ci) => {
        ...
        return (
          <rect key={ci} x={x1} y={PAD_T} width={x2 - x1} height={chartH} fill={cat.bg} rx={4} />
        );
      })}
      ...
      {[0, 25, 50, 75, 100].map(pct => {
        ...
        return (
          <line key={pct} x1={PAD_L} y1={y} x2={W - PAD_R} y2={y} stroke="rgba(200,185,255,0.25)" strokeWidth={1} />
        );
      })}
      ...
      {DRI_NUTRIENTS.map((n, i) => {
        ...
        return (
          <g key={n.name}>
            ...
            {/* 柱状图内的发育目标 emoji */}
            {v > 0 && n.goals.length > 0 && h > 8 && (
              <g>
                {n.goals.slice(0, 5).map((goalEmoji, gi) => {
  ...
  useEffect(() => {
    getChildren()
      .then(cs => {
        const c = cs[0] ?? null;
        setChild(c);
        if (c) setAgeIdx(stageIdxForChild(c.stageKey, c.age));
      })
      .catch(() => setChild(null));
  }, []);

  const childIdx = child ? stageIdxForChild(child.stageKey, child.age) : null;

  // 当前年龄段的目标按层分组
  const tiers = useMemo(() => {
    const out: Record<Tier, typeof DEV_GOALS> = { core: [], important: [], supporting: [] };
    DEV_GOALS.forEach(g => out[g.tiers[ageIdx]].push(g));
    return out;
  }, [ageIdx]);
        ...
        <section className="backdrop-blur-xl brightness-[1.08] rounded-[18px] border border-white/75 shadow-[0_8px_32px_rgba(120,80,200,0.12),0_2px_8px_rgba(120,80,200,0.06),inset_0_1.5px_0_rgba(255,255,255,0.95),inset_0_-1px_0_rgba(200,180,255,0.15),inset_1px_0_0_rgba(255,255,255,0.6),inset_-1px_0_0_rgba(255,255,255,0.6)] p-6 animate-fade-in-up relative overflow-hidden" style={{ background: 'rgba(255, 255, 255, 0.70)' }}>
          ...
          <div className="relative">
            <div className="flex items-center gap-2.5 mb-3">
              <h2 className="text-lg font-bold text-[#2d2a4a]">🎯 {isZh ? '各年龄段发育目标' : isEs ? 'Objetivos de Desarrollo por Etapa' : 'Development Goals by Age Stage'}</h2>
          ...
          <p className="text-[11px] font-extrabold uppercase tracking-wide text-gray-500 mb-2.5">
            ...
            <span className="ml-2 normal-case font-semibold text-gray-400 tracking-normal">· {isZh ? '点击阶段切换' : isEs ? 'Toca una etapa para explorar' : 'Tap a stage to explore'}</span>
          </p>
          ...
          <div className="grid gap-1 mb-4" style={gridCols}>
            {AGE_GROUPS.map((g, i) => (
              <button key={g.key} onClick={() => setAgeIdx(i)} className={`text-center ${i === ageIdx ? '' : 'opacity-60'}`}>
                ...
                <p className={`text-[10px] font-bold ${i === ageIdx ? 'text-fuchsia-600' : 'text-gray-500'}`}>{isZh ? g.nameZh : isEs ? g.nameEs : g.name}</p>
                {childIdx === i && child && (
                      ...
                      return (
                        <div key={g.id} className="rounded-xl px-3 py-2.5 border-[1.5px]" style={{ background: cardBg, borderColor: cardBorder }}>
                          <p className="flex items-center gap-1.5 mb-1.5">
                            ...
                            <span className="text-[13px] font-bold" style={{ color: cardTextColor }}>{isZh ? g.nameZh : isEs ? g.nameEs : g.name}</span>
                          </p>
                          <p className="flex flex-wrap gap-1">
                            {(isZh ? g.nutrientsZh : isEs ? g.nutrientsEs : g.nutrients).map(n => (
        ...
        <section className="backdrop-blur-xl brightness-[1.08] rounded-[18px] border border-white/75 shadow-[0_8px_32px_rgba(120,80,200,0.12),0_2px_8px_rgba(120,80,200,0.06),inset_0_1.5px_0_rgba(255,255,255,0.95),inset_0_-1px_0_rgba(200,180,255,0.15),inset_1px_0_0_rgba(255,255,255,0.6),inset_-1px_0_0_rgba(255,255,255,0.6)] p-6 animate-fade-in-up delay-100 relative overflow-hidden" style={{ background: 'rgba(255, 255, 255, 0.70)' }}>
          ...
          <div className="relative">
            <div className="flex items-center gap-2.5 mb-1">
              <h2 className="text-lg font-bold text-[#2d2a4a]">🥗 {isZh ? '每日营养指南' : isEs ? 'Guía Nutricional Diaria' : 'Daily Nutrition Guide'}</h2>
              <span className="ml-auto text-[11px] font-bold text-[#893ce3] bg-[rgba(137,60,227,0.12)] px-2.5 py-0.5 rounded-full">
                {isZh ? `${group.badgeZh} · DRI` : isEs ? `${group.badgeEs} · DRI` : `${group.badge} · DRI`}
              </span>
            </div>
            <p className="text-[11px] font-semibold text-gray-400 mb-3">
              {isZh ? '按年龄推荐的每日摄入量 · 与上方成长阶段联动' : isEs ? 'Ingesta diaria recomendada por edad · sincronizada con la etapa de crecimiento anterior' : 'Recommended daily intake by age · synced with Growth Stage above'}
            </p>
